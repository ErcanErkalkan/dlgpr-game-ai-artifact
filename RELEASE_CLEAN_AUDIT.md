...
Official release version: `v0.5.1`
...
The clean archive should be published from a repository tag named `v0.5.1`. The `.git/` directory is intentionally excluded from the artifact ZIP, so the tag is represented in the metadata and archive name rather than embedded as Git history.
